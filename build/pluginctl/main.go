...
func getClient() (*model.Client4, error) {
	...
	if adminUsername != "" && adminPassword != "" {
		...
		_, _, err := client.Login(context.TODO(), adminUsername, adminPassword)
		if err != nil {
...
func deploy(client *model.Client4, pluginID, bundlePath string) error {
	...
	_, _, err = client.UploadPluginForced(context.TODO(), pluginBundle)
	if err != nil {
		return fmt.Errorf("failed to upload plugin bundle: %s", err.Error())
	}

	log.Print("Enabling plugin.")
	_, err = client.EnablePlugin(context.TODO(), pluginID)
	if err != nil {
...
func disablePlugin(client *model.Client4, pluginID string) error {
	...
	_, err := client.DisablePlugin(context.TODO(), pluginID)
	if err != nil {
